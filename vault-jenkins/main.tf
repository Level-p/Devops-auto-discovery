...
resource "aws_acm_certificate" "steven-acm-cert" {
...
resource "aws_route53_record" "acm_validation_record" {
  for_each = {
    for dvo in aws_acm_certificate.steven-acm-cert.domain_validation_options : dvo.domain_name => {
  ...
  depends_on      = [aws_acm_certificate.steven-acm-cert]
}

# Validate the ACM Certificate after DNS Record Creation
resource "aws_acm_certificate_validation" "steven_cert_validation" {
  certificate_arn         = aws_acm_certificate.steven-acm-cert.arn
...
resource "aws_elb" "elb_jenkins" {
  ...
  listener {
    ...
    lb_port            = 443
    lb_protocol        = "HTTPS"
    ssl_certificate_id = aws_acm_certificate.steven-acm-cert.arn
  }
  health_check {
    healthy_threshold   = 3
...
resource "aws_instance" "vault" {
  ...
  key_name                    = aws_key_pair.public_key1.key_name # Use the created key pair
...
resource "aws_route53_record" "vault-record" {
  zone_id = data.aws_route53_zone.steven-acp-zone.id
